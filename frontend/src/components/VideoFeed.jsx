...
function VideoFeed({ videoUrl, isConnected, isStreaming }) {
  ...
  const [streamKey, setStreamKey]   = useState(0);   // ← forces img remount
  const prevStreaming                = useRef(false);

  useEffect(() => {
    if (isStreaming && !prevStreaming.current) {
      // Camera just turned ON — bump key so React unmounts/remounts the <img>
      // This kills the browser's stale MJPEG connection entirely
      setStreamKey(k => k + 1);
      setIsLoading(true);
      setHasError(false);
    }
  ...
  return (
    <div className="neo-card p-3">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-400">Hand gesture detection</p>
          </div>
        </div>

        <div className="flex items-center gap-1.5">
          <span className={`w-2 h-2 rounded-full ${isConnected && isStreaming ? "bg-accent-success status-pulse" : "bg-gray-500"}`} />
          <span className={`text-xs font-medium ${isConnected && isStreaming ? "text-accent-success" : "text-gray-500"}`}>
            {isConnected && isStreaming ? "LIVE" : "OFFLINE"}
          </span>
        </div>
      </div>

      {/* Video container — aspect-video keeps it proportional */}
      <div className="video-container aspect-video bg-dark-800 relative">
          ...
          {isLoading && isStreaming && (
            <motion.div key="loading"
              ...
              <div className="text-center">
                <div className="w-10 h-10 border-4 border-accent-primary border-t-transparent rounded-full animate-spin mx-auto mb-2" />
                <p className="text-gray-400 text-xs">Connecting to camera...</p>
              </div>
            </motion.div>
          )}
          ...
          {hasError && (
            <motion.div key="error"
              ...
              <div className="text-center p-4">
                <div className="w-12 h-12 rounded-full bg-accent-danger/20 flex items-center justify-center mx-auto mb-3">
                  <svg className="w-6 h-6 text-accent-danger" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                      d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                  </svg>
                </div>
                <p className="text-gray-300 font-medium text-sm mb-1">Connection Failed</p>
                <p className="text-gray-500 text-xs">Unable to connect to video stream</p>
              </div>
            </motion.div>
          )}
          ...
          {!isStreaming && (
            <motion.div key="stopped"
              ...
              <div className="text-center p-4">
                ...
                <p className="text-gray-300 font-medium text-sm mb-1">Camera Stopped</p>
                <p className="text-gray-500 text-xs">Click Start to begin detection</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>

        {/* key={streamKey} forces React to unmount+remount the img element,
            which kills the stale MJPEG TCP connection in the browser */}
        {isStreaming && (
          <img
            key={streamKey}
            src={`${videoUrl}?s=${streamKey}`}
            alt="Live hand gesture detection feed"
            className={`w-full h-full object-contain rounded-xl transition-opacity duration-300 ${isLoading || hasError ? "opacity-0" : "opacity-100"}`}
            onLoad={handleLoad}
            onError={handleError}
          />
        )}
        ...
        {/* Corner brackets */}
        <div className="absolute top-3 left-3 w-6 h-6 border-l-2 border-t-2 border-accent-primary/50 rounded-tl pointer-events-none" />
        <div className="absolute top-3 right-3 w-6 h-6 border-r-2 border-t-2 border-accent-primary/50 rounded-tr pointer-events-none" />
        <div className="absolute bottom-3 left-3 w-6 h-6 border-l-2 border-b-2 border-accent-primary/50 rounded-bl pointer-events-none" />
        <div className="absolute bottom-3 right-3 w-6 h-6 border-r-2 border-b-2 border-accent-primary/50 rounded-br pointer-events-none" />
      </div>
    </div>
  );
}
